...
bool initScene1(HWND hWnd)
{
    ...
    kSprBackGround=new JupiterSprite(hDCTmp,kAmbiousPtr->getWidth(),kAmbiousPtr->getHeight(),RGB(40,80,160));
...
{	
    ...
    for(int i=0;i<8;i++) {	// Clouds position and z order.
        ...
        if(kCloudPt[i].y > kAmbiousPtr->getHeight()) {
            ...
            kCloudPt[i].x = rand()%kAmbiousPtr->getWidth()-kAmbiousPtr->getWidth()/2;
