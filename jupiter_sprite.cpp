 ...
 *                  dependencies msing32.lib, gdi32.lib
 *                  Have fun! :-)
 */
...
BOOL Sprite::create(HDC hDC, int nWidth, int nHeight,
...
void Sprite::draw(HDC hDC, int x, int y, bool bTrans/* = false*/, 
                  COLORREF crTrans/* = RGB(0, 0, 0)*/)
{
    if (spriteBmp != NULL) {
        ...
        if (bTrans)
             TransparentBlt(hDC, x, y, m_nWidth, m_nHeight, hdcMem, 0, 0,
             m_nWidth, m_nHeight, crTrans);
        else
...
void Sprite::drawRect(HDC hDC, int x, int y, int nRectX, int nRectY,
...
void Sprite::drawAlpha(HDC hDC,int x, int y, int nAlpha)
{
    if (spriteBmp != NULL) {
        ...
        bf.AlphaFormat=0x00;
        AlphaBlend(hDC, x, y, m_nWidth, m_nHeight, hdcMem, 0, 0,GetWidth(),
            GetHeight(), bf);
...
void Sprite::drawTransAlpha(HDC hdcBack, int nXBack, int nYBack, 
...
void Sprite::transBitmap(HDC hdcDest, int nXDest, int nYDest, int nWidthDest,
